    ...
    <nav className="fixed bottom-0 left-0 right-0 h-24 bg-[#020617]/95 backdrop-blur-2xl border-t border-white/10 px-6 flex items-center justify-between z-50 w-full">
      {navItems.map((item) => {
        ...
        return (
          <button 
            key={item.id}
            onClick={() => (item.path.startsWith('/') && item.id !== 'prediction' && item.id !== 'account') ? navigate(item.path) : null}
            className={`flex flex-col items-center gap-1.5 transition-all duration-300 flex-1 ${isActive ? 'text-[#00D8FF]' : 'text-slate-500 opacity-40'}`}
          >
            <Icon size={24} strokeWidth={isActive ? 2.5 : 2} />
            <span className="text-[8px] font-black uppercase tracking-widest">{item.label}</span>
            {isActive && <div className="absolute -bottom-1 w-1 h-1 bg-[#00D8FF] rounded-full shadow-[0_0_10px_#00D8FF]" />}
          </button>
        );
      })}
    </nav>
